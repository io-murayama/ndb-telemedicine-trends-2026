...
- 地図（Figure D）は `data/reference/prefectures.geojson`（[dataofjapan/land](https://github.com/dataofjapan/land)）を用いた choropleth。**色は log10 スケール**（東京都の外れ値を抑えて地域差を可視化）。
- **Supplementary Figure E** は、2022→2024 の人口あたり算定回数の **絶対差**（10万人あたり）を都道府県別にロリポップで示す（Figure 4 と同型）。増加が大きい県と小さい県の地域差を比較できる。
- **Supplementary Figure F** は、同じ期間の **相対変化率（%）** を地図 heatmap で示す。減少はグレー、増加は緑のグラデーション（凡例は −50～200% でクリップ）。
