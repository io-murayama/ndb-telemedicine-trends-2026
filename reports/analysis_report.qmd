...
### 3.3a 年齢・初診／再診別の増加率（Figure 4）

![Figure 4. 年齢階級別増加率（2022→2024）](../output/figures/figure4_change_by_age_visit.png)

```{r change-table}
change <- read_tab("change_2022_2024_by_age.csv")
change$相対増加率 <- sprintf("%.1f%%", change$relative_change_pct)
change$絶対差 <- sprintf("%.3f pp", change$abs_change_pp)
change$種別 <- ifelse(change$visit_type == "initial", "初診", "再診・外来")
knitr::kable(
  change[order(change$age_group), c("age_group", "種別", "相対増加率", "絶対差")],
  col.names = c("年齢階級", "種別", "相対増加率（2022→2024）", "絶対差（pp）"),
  caption = "表2a. 年齢階級・初診／再診別の増加（2022→2024）"
)
```

#### 解釈

- **全年齢帯で増加**。相対増加率は初診・再診ともに高いが、**若年層（とくに 0～4 歳、20～30 代）で増加率が大きい** 傾向。
- 初診と再診・外来で **増加パターンの形は類似** するが、水準（絶対割合）は初診の方が高い（Figure 2 参照）。
...
pref2024 <- pref2024[order(-pref2024$rate_per_population), ]
pref2024$率 <- sprintf("%.1f", pref2024$rate_per_population)
knitr::kable(
  rbind(
    head(pref2024[, c("prefecture_name", "率")], 5),
    tail(pref2024[, c("prefecture_name", "率")], 5)
  ),
...
![Supplementary Figure C. 都道府県別人口あたり（2022–2024 平均）](../output/figures/supplementary_prefecture_per_capita_pooled.png)

#### 解釈

- **医療機関所在地** に基づく都道府県別の **人口あたり ICT 算定回数**（10万人あたり）。
- 分母は **総務省統計局人口推計**（都道府県別総人口）。`data/reference/prefecture_population.csv` を参照。
- 年齢・性別標準化と異なり、**地域の人口規模を考慮した利用率** として解釈できる。ただし医療機関所在地と患者居住地は一致しない場合がある（SAP 6）。
