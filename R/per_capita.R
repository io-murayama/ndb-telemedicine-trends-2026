...
PREFECTURE_VISIT_SCOPES <- list(
  initial = list(
    label = "初診",
    metrics = c("online_initial")
  ),
  followup = list(
    label = "再診・外来",
    metrics = c("online_followup", "online_outpatient")
  ),
  total = list(
    label = "合計",
    metrics = c("online_initial", "online_followup", "online_outpatient")
  )
)
...
aggregate_prefecture_online <- function(cross_df, metric_types = NULL) {
  if (is.null(metric_types)) {
    metric_types <- PREFECTURE_VISIT_SCOPES$total$metrics
  }
  online <- cross_df[cross_df$metric_type %in% metric_types, , drop = FALSE]
  aggregate(
...
prepare_per_capita_table <- function(cross_df, pop_df, per = 100000, visit_scope = "total") {
  scope <- PREFECTURE_VISIT_SCOPES[[visit_scope]]
  if (is.null(scope)) {
    stop("Unknown visit_scope: ", visit_scope, call. = FALSE)
  }
  online <- aggregate_prefecture_online(cross_df, metric_types = scope$metrics)
  ...
  merged$visit_scope <- visit_scope
  merged$visit_scope_label <- scope$label
  merged
}

prepare_all_prefecture_per_capita <- function(cross_df, pop_df, per = 100000) {
  pieces <- lapply(names(PREFECTURE_VISIT_SCOPES), function(scope) {
    prepare_per_capita_table(cross_df, pop_df, per = per, visit_scope = scope)
  })
...
aggregate_prefecture_change_2022_2024 <- function(per_capita_df, baseline_year = 2022, end_year = 2024) {
  scopes <- unique(per_capita_df$visit_scope)
  pieces <- lapply(scopes, function(scope) {
    scope_df <- per_capita_df[per_capita_df$visit_scope == scope, , drop = FALSE]
    aggregate_prefecture_change_scope(scope_df, baseline_year = baseline_year, end_year = end_year)
  })
  do.call(rbind, pieces)
}

aggregate_prefecture_change_scope <- function(per_capita_df, baseline_year = 2022, end_year = 2024) {
  ...
  if ("visit_scope" %in% names(per_capita_df)) {
    merged$visit_scope <- per_capita_df$visit_scope[1]
    merged$visit_scope_label <- per_capita_df$visit_scope_label[1]
  }
